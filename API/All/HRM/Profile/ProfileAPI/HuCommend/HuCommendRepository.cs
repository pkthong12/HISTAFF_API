...
{
    public class HuCommendRepository : IHuCommendRepository
    {
        private readonly GenericUnitOfWork _uow;
        private readonly FullDbContext _dbContext;
        private IGenericRepository<HU_COMMEND, HuCommendDTO> _genericRepository;
        private readonly GenericReducer<HU_COMMEND, HuCommendDTO> _genericReducer;

        public HuCommendRepository(FullDbContext context, GenericUnitOfWork uow)
        {
            _dbContext = context;
            _uow = uow;
            _genericRepository = _uow.GenericRepository<HU_COMMEND, HuCommendDTO>();
            _genericReducer = new();
        }
        ...
        public async Task<FormatedResponse> Create(GenericUnitOfWork _uow, HuCommendDTO dto, string sid)
        {
            var response = await _genericRepository.Create(_uow, dto, sid);
            return response;
        }

        public async Task<FormatedResponse> CreateRange(GenericUnitOfWork _uow, List<HuCommendDTO> dtos, string sid)
        {
            var add = new List<HuCommendDTO>();
            add.AddRange(dtos);
        ...
        public async Task<FormatedResponse> UpdateRange(GenericUnitOfWork _uow, List<HuCommendDTO> dtos, string sid, bool patchMode = true)
        {
        ...
        public async Task<FormatedResponse> ApproveCommend(List<long> Ids)
        {
            try
            {
                ...
                Ids.ForEach(item =>
                {
                    ...
                    list.Add(new HuCommendDTO()
                    {
                        Id = response.ID,
                        StatusPaymentId = response.STATUS_PAYMENT_ID
                    });
                });
