...
{
    [ApiExplorerSettings(GroupName = "093-PROFILE-HU_EVALUATE")]
    [ApiController]
    [HiStaffAuthorize]
    [Route("api/[controller]/[action]")]
    public class PortalEvaluateController : ControllerBase
    {
        private readonly GenericUnitOfWork _uow;
        private readonly IPortalEvaluateRepository _HuEvaluateRepository;
        private readonly AppSettings _appSettings;

        public PortalEvaluateController(
            FullDbContext dbContext,
            IOptions<AppSettings> options)
        {
            _uow = new GenericUnitOfWork(dbContext);
            _HuEvaluateRepository = new PortalEvaluateRepository(dbContext, _uow);
        ...
        {
            var response = await _HuEvaluateRepository.GetByEmployeeId(employeeId);
            return Ok(response);
        }
    }
}
