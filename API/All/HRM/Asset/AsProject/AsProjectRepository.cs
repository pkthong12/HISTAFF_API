...
{
    public class AsProjectRepository : IAsProjectRepository
    {
        private readonly GenericUnitOfWork _uow;
        private readonly FullDbContext _dbContext;
        private IGenericRepository<AS_PROJECT, AsProjectDTO> _genericRepository;
        private readonly GenericReducer<AS_PROJECT, AsProjectDTO> _genericReducer;

        public AsProjectRepository(FullDbContext context, GenericUnitOfWork uow)
        {
            _dbContext = context;
            _uow = uow;
            _genericRepository = _uow.GenericRepository<AS_PROJECT, AsProjectDTO>();
            _genericReducer = new();
        }

        public async Task<GenericPhaseTwoListResponse<AsProjectDTO>> SinglePhaseQueryList(GenericQueryListDTO<AsProjectDTO> request)
        {
            var joined = (from p in _dbContext.AsProjects
                          select new AsProjectDTO
                          {
                              Id = p.ID,
                              Code = p.CODE,
                              Name = p.NAME,
        ...
        public async Task<FormatedResponse> Create(GenericUnitOfWork _uow, AsProjectDTO dto, string sid)
        {
            // set "IsActive = true"
        ...
        public async Task<FormatedResponse> CreateRange(GenericUnitOfWork _uow, List<AsProjectDTO> dtos, string sid)
        {
            var add = new List<AsProjectDTO>();
            add.AddRange(dtos);
            var response = await _genericRepository.CreateRange(_uow, add, sid);
            return response;
        }

        public async Task<FormatedResponse> Update(GenericUnitOfWork _uow, AsProjectDTO dto, string sid, bool patchMode = true)
        {
            var response = await _genericRepository.Update(_uow, dto, sid, patchMode);
            return response;
        }

        public async Task<FormatedResponse> UpdateRange(GenericUnitOfWork _uow, List<AsProjectDTO> dtos, string sid, bool patchMode = true)
        {
